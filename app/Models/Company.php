    ...
    */
    ...
    public static function getIdForCurrentUser($unescaped_input)
    {
        if (!static::isFullMultipleCompanySupportEnabled()) {
            ...
            // NOTE by R.Florek: Updated for Admin role scope
    ...
    public static function isCurrentUserHasAccess($companyable)
    {
        if (is_null($companyable)) {
            ...
            if (Auth::user()) {
                ...
                // NOTE by R.Florek: Updated for MultiCompany role scope
                return ($current_user_company_id == null || $current_user_company_id == $companyable_company_id || Auth::user()->isSuperUser() || Auth::user()->isMultiCompany());
            }
    ...
    {
        // NOTE by R.Florek: Updated for MultiCompany role scope
        return ((!static::isFullMultipleCompanySupportEnabled()) || (Auth::user()->isSuperUser()) || Auth::user()->isMultiCompany());
    }
    ...
    {
        // NOTE by R.Florek: Updated for MultiCompany role scope
        if (!static::isFullMultipleCompanySupportEnabled() || Auth::user()->isSuperUser() || Auth::user()->isMultiCompany()) {
            return static::getIdFromInput($unescaped_input);
        } else {
    ...
    public static function scopeCompanyables($query, $column = 'company_id', $table_name = null )
    {
        ...
        // NOTE by R.Florek: Updated for MultiCompany role scope
        if (!static::isFullMultipleCompanySupportEnabled() || (Auth::check() && (Auth::user()->isSuperUser() || Auth::user()->isMultiCompany())) || (!Auth::check())) {
            return $query;
        } else {
    ...
    {
        // NOTE by R.Florek: Updated for MultiCompany role scope
        // NOTE by R.Florek: I realy don't know what it is
        if (count($companyable_names) == 0) {
            throw new Exception('No Companyable Children to scope');
        } elseif (!static::isFullMultipleCompanySupportEnabled() || (Auth::check() && (Auth::user()->isSuperUser() || Auth::user()->isMultiCompany()))) {
            return $query;
        } else {
            $f = function ($q) {
